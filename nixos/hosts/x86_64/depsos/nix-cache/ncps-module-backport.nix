# https://github.com/NixOS/nixpkgs/blob/8110df5ad7abf5d4c0f6fb0f8f978390e77f9685/nixos/modules/services/networking/ncps.nix
{
  config,
  pkgs,
  lib,
  ...
  ncpsWrapper = pkgs.writeShellScript "ncps-wrapper" ''
    ...
    ${lib.optionalString (cfg.cache.redis != null) (
      if cfg.cache.redis.passwordFile != null
      then ''export CACHE_REDIS_PASSWORD="$(cat "$CREDENTIALS_DIRECTORY/redisPassword")"''
      else if cfg.cache.redis.password != null
      then ''export CACHE_REDIS_PASSWORD="${cfg.cache.redis.password}"''
      else ""
    )}
  ...
  dbPath =
    if isSqlite
    then lib.removePrefix "sqlite:" cfg.cache.databaseURL
    else null;
  dbDir =
    if isSqlite
    then dirOf dbPath
...
in {
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.tmpfiles.settings.ncps = let
      perms = {
        group = "ncps";
        mode = "0700";
        user = "ncps";
